...
namespace Batheer.WebUI.Areas.Researcher.Controllers
{
    ...
    public class ManageFamilyMembersController : BaseController
    {
        ...
        public async Task<IActionResult> Index(Guid familyObjectkey)
        {
            ...
            var model = await Mediator.Send(new Application.FamilyMembers.Queries.GetByFamilyObjectkey
        ...
        public async Task<IActionResult> Edit(Guid familyMemberObjectkey)
        {
            var item = await Mediator.Send(new GetFamilyMemberByObjectkeyQuery(familyMemberObjectkey));
            ViewBag.Item = item;

            await Create_fillData();

            var config = new MapperConfiguration(cfg => cfg.CreateMap<Application.FamilyMembers.Queries.GetFamilyMemberByObjectkey.FamilyMemberDto, UpdateFamilyMemberCommand>());
        ...
        public async Task<IActionResult> Edit(UpdateFamilyMemberCommand command, Guid familyMemberObjectkey)
        {
            if (ModelState.IsValid)
            {
                await Mediator.Send(command);
                return RedirectToAction(nameof(Edit), new { familyMemberObjectkey });
            }

            ViewBag.FamilyMemberObjectkey = familyMemberObjectkey;
        ...
        [HttpPost]
        public async Task<IActionResult> VisitingFamilyMemberResult(SaveVisitingFamilyMemberResultCommand command, Guid familyMemberObjectkey)
        {
            ...
            {
                await Mediator.Send(command);
                return RedirectToAction(nameof(Edit), new { familyMemberObjectkey = familyMemberObjectkey });
            }
