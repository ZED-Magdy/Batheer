...
{
    public class SP_SearchInFamilyRegistrationRequests : Migration
    {
...
BEGIN
	...
	SELECT
		...
		IIF(rfm.Id IS not NULL, N'عائل الأسرة', N'فرد بالأسرة') FamilyMemberTypeName,
	...
	LEFT JOIN dbo.PersonalDataForm p WITH(NOLOCK) ON p.Id = ISNULL( rfm.PersonalDataFormId , fm.PersonalDataFormId)
